...
class IfNode:
    def __init__(self, cases, else_case) -> None:
        ...
        # get position from first case's expression, first 0 for first case, and second zero for expression
...
class Parser:
    ...
    def atom(self):
        ...
        if tok.type in (TT_INT, TT_FLOAT):
            ...
            while_expr = res.register(self.if_expr())
            if res.error:
                return res
            return res.success(while_expr)

        elif tok.matches(TT_KEYWORD, 'FOR'):
            while_expr = res.register(self.for_expr())
            if res.error:
                return res
            return res.success(while_expr)
